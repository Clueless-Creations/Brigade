...
- Original author: TypeSafe AI, publishing organization for docs, hosted System One API, and client SDKs
- License: MIT (verified)
- Canonical URL: https://github.com/typesafe-ai/typesafe-sdk-python
- Contributes: Candidate semantic-assessment provider (TypeSafe System One / Jev). Brigade records qualification mapping and retains the Python SDK MIT notice; it does not install the SDK, call the API, or claim live support in this unit.
- How we consume it:
  - remote-service: Qualify the hosted System One evaluation API (POST /v1/systemone) as a candidate semantic-assessment provider against landed SQ-01 contracts. Paper qualification only — no live call, no Brigade package dependency, no credential invent, no adapter transport.
  - adapted-method: Reauthor native Choice/Score/Noul (+ batch) shapes from official public docs into a Brigade-owned mapping matrix targeting contracts/semantic/. No upstream SDK types are imported into policy. Independent fixtures preserve official-doc JSON examples with provenance; they are not adapter-authored expected-native captures.
