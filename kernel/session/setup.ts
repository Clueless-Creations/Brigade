...
function main(): number {
  ...
  console.log(
    [
      ...
      `  2. Read current state: b2c business-status --workspace <slug> --json`,
      `  3. Inspect the plan: b2c business-plan --workspace <slug> --json`,
      `  4. Research it. After explicit acceptance in product.yaml, run b2c render-product --workspace <slug>.`,
      `  5. Read a fresh revision: b2c business-plan --workspace <slug> --json`,
      `  6. Initialize: b2c business-initialize --workspace <slug> --revision <revision-from-plan> --json`,
      "     Initialization grants no work authority. Record approved authority with b2c onboard before business-run.",
      "  Existing registered workspace: resume with business-status then business-plan. Existing unregistered scaffold: b2c workspaces register <slug> <path>.",
      "  Legacy b2c new and b2c bootstrap remain supported for explicit scaffold and runtime maintenance.",
      "  Catalog discovery and composition preview are optional later:",
      "  b2c catalog --json; b2c compose --config b2c.yaml --json",
