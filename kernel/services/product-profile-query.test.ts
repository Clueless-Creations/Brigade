...
test("unknowns can be targeted without loading the whole profile",()=>withWorkspace(root=>{const result=queryProductProfile(root,{ids:["unknown:state"],maxRecords:8});assert.ok(result.records.some((r:any)=>r.id==="unknown:state"));assert.ok(result.records.length<=8)}));
