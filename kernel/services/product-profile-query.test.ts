...
test("unknowns can be targeted without loading the whole profile",()=>withWorkspace(root=>{const result=queryProductProfile(root,{text:"unknown",maxRecords:8});assert.ok(result.records.length<=8);assert.ok(result.records.some((r:any)=>String(r.id).startsWith("unknown:") || String(r.id).startsWith("coverage:"))); assert.ok(result.records.length<=8)}));
