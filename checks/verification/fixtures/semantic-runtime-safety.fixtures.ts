 ...
 *
 * Also: modes+rollback; NO_523_IMPL cleared; NEXT_AFTER=#574; no new daemon.
 */
...
export function register(harness: Harness): void {
  harness.check("semantic-runtime-safety: stamp/issue/consumes + AC map + hard bans + NO_523 cleared", () => {
    ...
    // Tip stamps: NO_523 cleared; NEXT_AFTER advanced to #528.
    ...
    assert(SEMANTIC_RUNTIME_SAFETY_NEXT_AFTER_CLOSE === "#574", "map next #511");
    assert(SERVICE_NEXT_AFTER === "#574", "service next #511");
    assert(SEMANTIC_RUNTIME_SAFETY_ROLLOUT_NEXT_AFTER_CLOSE === "#574", "rollout next #511");
    ...
    assert(KNOWLEDGE_APPLICABILITY_NEXT_AFTER_CLOSE === "#574", "applicability next #511");
    assert(CONTEXT_BOUND_APPLICABILITY_NEXT_AFTER_CLOSE === "#574", "beam next #511");
    assert(APPLICABILITY_PATH_PROJECTION_NEXT_AFTER_CLOSE === "#574", "projection next #511");
    assert(FEEDBACK_TO_WORK_SHADOW_NEXT_AFTER_CLOSE === "#574", "522 map next #511");
    assert(SERVICE_522_NEXT === "#574", "522 service next #511");
    assert(WORK_PROPOSAL_SHADOW_NEXT_AFTER_CLOSE === "#574", "proposal next #511");
    assert(FEEDBACK_TO_WORK_SHADOW_RECIPE_NEXT_AFTER_CLOSE === "#574", "recipe next #511");
